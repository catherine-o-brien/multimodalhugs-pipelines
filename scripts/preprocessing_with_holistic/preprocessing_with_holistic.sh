...
poses=$data/poses/holistic
preprocessed=$data/preprocess/holistic
...
if [[ -s $preprocessed/rwth_phoenix2014_t.train.tsv ]]; then
    echo "Preprocessed file exists: $preprocessed/holistic/rwth_phoenix2014_t.train.tsv"
    echo "Skipping"
    exit 0
else
...
python $scripts/preprocessing_with_holistic/preprocessing_with_holistic.py \
    --pose-dir $poses \
